...
def sub_ncerr(left,right):
    ...
    def sub_som_som(som1,som2):
        # check that there is the same number of so
        if len(som1)!=len(som2):
            raise IndexError,"Can only sub SOMs with same number of spectra"
        ...
        for (so1,so2) in map(None,som1,som2):
            result.append(sub_so_so(so1,so2))
        return result

    def sub_som_so(som,so):
        # create empty result som
        result=SOM.som.SOM()
        ...
        # do the calculation
        for it in som:
            result.append(sub_so_so(it,so))
        return result

    def sub_so_som(so,som):
        # create empty result som
        result=SOM.som.SOM()
        ...
        # do the calculation
        for it in som:
            result.append(sub_so_so(so,it))
        return result

    def sub_som_num(som,num):
        # create empty result som
        result=SOM.som.SOM()
        ...
        # do the calculation
        for it in som:
            result.append(sub_so_num(it,num))
        return result

    def sub_num_som(num,som):
        # create empty result som
        result=SOM.som.SOM()
        ...
        # do the calculation
        for it in som:
            result.append(sub_num_so(num,it))
        return result

    def sub_so_so(so1,so2):
        if so1.x!=so2.x:
            raise RunTimeError,"x axis must be equal to sub spectra"
        ...
        # do the math
        (result.y,result.var_y)=array_manip.sub_ncerr(so1.y,so1.var_y,
                                                      so2.y,so2.var_y)

        return result

    def sub_so_num(so,num):
        # set up the result
        result=SOM.so.SO()
        result.id=so.id
        result.x=so.x
        ...
        # do the math
        (result.y,result.var_y)=array_manip.sub_ncerr(so.y,so.var_y,
                                                      num[0],num[1])

        return result

    def sub_num_so(num,so):
        # set up the result
        result=SOM.so.SO()
        result.id=so.id
        result.x=so.x
        ...
        # do the math
        (result.y,result.var_y)=array_manip.sub_ncerr(num[0],num[1],
                                                      so.y,so.var_y)
    ...
    try:
        ...
        try:
            ...
            return sub_som_som(left,right)
        except AttributeError:
            try:
                right.id
                return sub_som_so(left,right)
            except AttributeError:
                return sub_som_num(left,right)
    ...
    try:
        ...
        try:
            right.attr_list[TITLE]
            return sub_so_som(left,right)
        except AttributeError:
            try:
                right.id
                return sub_so_so(left,right)
            except AttributeError:
                return sub_so_num(left,right)
    except AttributeError:
        pass
    ...
    try:
        right.attr_list[TITLE]
        return sub_num_som(left,right)
    except AttributeError:
        right.id
        return sub_num_so(left,right)
...
if __name__=="__main__":
    ...
    print "********** sub_ncerr"
    print "* so  -scal:",so_to_str(sub_ncerr(som1[0],(1,1)))
    print "* scal-so  :",so_to_str(sub_ncerr((1,1),som1[0]))
    print "* so  -so  :",so_to_str(sub_ncerr(som1[0],som1[1]))
