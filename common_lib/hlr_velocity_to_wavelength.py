...
def velocity_to_wavelength(obj, **kwargs):
    """
    This function converts a primary axis of a C{SOM} or C{SO} from velocity to
    wavelength. The velocity axis for a C{SOM} must be in units of
    I{meters/microseconds}. The primary axis of a C{SO} is assumed to be in
    units of I{meters/microseconds}. A C{tuple} of C{(velocity, velocity_err2)}
    (assumed to be in units of I{meters/microseconds}) can be converted to
    C{(wavelength, wavelength_err)}.
    ...
    @keyword units: The expected units for this function. The default for this
                    function is I{meters/microseconds}
    @type units: C{string}


    @return: Object with a primary axis in velocity converted to wavelength
    ...
    if res_descr == "SOM":
        ...
        result.setYUnits("Counts/Angstroms")
    ...
    for i in xrange(len_obj):
        ...
        value = axis_manip.velocity_to_wavelength(val, err2)
...
if __name__ == "__main__":
    ...
    som1.setAllAxisUnits(["meters/microseconds"])

    som2 = hlr_test.generate_som()
    som2.setAllAxisUnits(["meters/microseconds"])
    ...
    print "********** velocity_to_wavelength"
    print "* som  :", velocity_to_wavelength(som1)
    print "* so   :", velocity_to_wavelength(som1[1])
    print "* scal :", velocity_to_wavelength((1, 1))
